...
num_T_period=4;     % number of periods to simulate (total simulation time)
                    %  It should be large enough to get into steady state
wave_option=0;      %0: measured water partical velocity from Luhar
                    %1: linear wave theory
...
elseif wave_option < 2
    Ux_wave=@(x,z,t) Height_wave/2*Omega_wave*cosh(k_wave_number.*z)/sinh(k_wave_number*Depth_water).*cos(k_wave_number.*x-Omega_wave.*t+crest*pi) + Uc;
    Uz_wave=@(x,z,t) Height_wave/2*Omega_wave*sinh(k_wave_number.*z)/sinh(k_wave_number*Depth_water).*sin(k_wave_number.*x-Omega_wave.*t+crest*pi) + Uc;
...
%% save data
file=['Foam_Dt',num2str(Dt*1e3),'ms_n',num2str(n)];
direction=['.\LuharComparisonResults\',file];
mkdir(direction);
Filename=[file,'.mat'];
...
%% comparison with Luhar and Nepf (2016)
figure(1)
clf;
T_num=T_wave/Dt;
DT=round(T_num/64);
...
order_T=1;
hold on
plt = plot(Ex(:,1),Ex(:,2),'ko',Num(:,1),Num(:,2),'r:',DisX(n+1,num_time+1-order_T*T_num:num_time-(order_T-1)*T_num),DisZ(n+1,num_time+1-order_T*T_num:num_time-(order_T-1)*T_num),'g-','LineWidth',2.5);
